...
- Use **Open data** when comparing published datasets. Process profiles and LCIA detail rows in
  ...
  publishing results. Data Processing preview also shows the Calculation Bundle contract, coverage,
  snapshot / method / factor hashes, and downloadable artifacts; downloads refresh secure links and
  verify byte size plus SHA-256 before saving. If links expire, artifact metadata changes, or that
  evidence is incomplete or blocked, Open Data analysis does not treat it as a publishable result.
